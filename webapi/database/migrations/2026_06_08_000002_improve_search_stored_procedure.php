...
return new class extends Migration
{
    ...
    {
        DB::unprepared('DROP PROCEDURE IF EXISTS sp_search_provider_services');
...
SQL);
    ...
    {
        // Restaurar versión anterior (solo busca título, descripción, categoría y negocio)
        DB::unprepared('DROP PROCEDURE IF EXISTS sp_search_provider_services');
...
BEGIN
  SELECT
    ps.id AS service_id, ps.title, ps.description, ps.base_price, ps.price_type,
    c.id AS category_id, c.name AS category_name,
    pp.id AS provider_profile_id,
    COALESCE(pp.business_name, u.full_name) AS provider_name,
    pp.whatsapp, pp.contact_phone, pp.address_text,
    ...
    d.latitude AS provider_latitude, d.longitude AS provider_longitude,
  ...
  WHERE ps.is_active = 1 AND c.is_active = 1 AND u.status = 'activo'
    AND (p_category_id IS NULL OR ps.category_id = p_category_id)
    AND (p_district_id IS NULL OR d.id = p_district_id)
    AND (p_keyword IS NULL
      ...
      OR c.name LIKE CONCAT('%', p_keyword, '%')
      OR COALESCE(pp.business_name, u.full_name) LIKE CONCAT('%', p_keyword, '%'))
    AND (CASE WHEN p_user_lat IS NULL OR p_user_lng IS NULL OR p_radius_km IS NULL THEN TRUE
         ELSE fn_distance_km(p_user_lat, p_user_lng, d.latitude, d.longitude) <= p_radius_km END)
  ORDER BY
    CASE WHEN p_user_lat IS NULL OR p_user_lng IS NULL THEN 0
    ELSE fn_distance_km(p_user_lat, p_user_lng, d.latitude, d.longitude) END ASC,
    pp.avg_rating DESC, pp.total_reviews DESC, ps.created_at DESC;
END
SQL);
    }
};
